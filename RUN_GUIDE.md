...
## 四、近三日价格展示（已于 2026-08 废弃）

原「近三日对比」产品线已彻底下线：不再生成 `SMM锂电现货价格_近三日对比_*.xlsx`，相关代码（`price_statistics.py`）、配置（`rolling_price_export`）与历史派生文件均已删除。每日 Excel 只展示当日数据，首页展示最近 7 个业务日期的正式文件。
