...
def generate_daily_report(meta: dict, sync_stats: dict | None = None,
    ...
    """从采集元数据和可选同步统计生成数据质量报告 dict。

    meta: collect() 返回的元数据 dict
    sync_stats: synchronizer.sync() 返回的统计 dict（可选）
    """
    ...
    if output_dir:
        ...
        out = output_dir / f"{target[:4]}" / f"{target[5:7]}"
